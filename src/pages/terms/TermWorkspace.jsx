...
export default function TermWorkspace() {
  ...
  const archive = async () => {
    if (
      !confirm(
        "Archive this term? It will become read-only. You can still view its report cards.",
      )
    )
      return;
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl">
      <button
        onClick={() => nav("/terms")}
        className="text-sm text-ink-soft hover:text-ink flex items-center gap-1 mb-4"
      >
      ...
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        ...
        <div className="flex gap-2">
          {!isCurrent && !isArchived && (
            <Button variant="secondary" onClick={setCurrent}>
              Set as current
            </Button>
          )}
          {!isArchived && (
      ...
      <TabBar
        tabs={TABS}
        activeTab={activeTab}
        onSelect={(id) => nav(`/terms/${termId}/${id}`)}
      />
      ...
      {activeTab === "scores" && (
        <ScoresTab term={term} readOnly={isArchived} />
      )}
      {activeTab === "attendance" && (
        <AttendanceTab term={term} readOnly={isArchived} />
      )}
      {activeTab === "traits" && (
        <TraitsCommentsTab term={term} readOnly={isArchived} />
      )}
      {activeTab === "reports" && (
        <ReportCardsTab term={term} readOnly={isArchived} />
      )}
      {activeTab === "roster" && <RosterTab term={term} />}
      {activeTab === "settings" && (
        <TermSettingsTab term={term} readOnly={isArchived} />
      )}
    </div>
  );
}
