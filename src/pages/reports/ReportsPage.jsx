...
export default function ReportsPage() {
  ...
  const [reportCards, setReportCards] = useState([]) // published/saved snapshots
  ...
  // Load results for the class + term (needed for live computation of class positions/averages)
  useEffect(() => {
  ...
  // Load existing (published) report card snapshots
  useEffect(() => {
  ...
  // Compute live reports for the whole class (used for previews)
  const computedReports = useMemo(() => {
    if (!classId || !termId || students.length === 0) return new Map()
    const classSubjects = subjects.filter(s => (s.classIds || []).includes(classId) && s.active)
  ...
  if (isParent) {
    return <ParentView db={db} profile={profile} school={school} terms={terms} />
  }
          ...
          <Select label="Term" value={termId} onChange={e => setTermId(e.target.value)}>
            ...
            {terms.map(t => (
              <option key={t.id} value={t.id}>
                ...
                {t.id === school?.currentTermId ? ' (current)' : ''}
              </option>
            ))}
          </Select>
                      ...
                      {computed && (
                        <>
                          ...
                          {computed.overallPosition && <>
                            <span>·</span>
                            <span>Position: {computed.overallPosition}</span>
                          </>}
                        </>
                      )}
                    </div>
...
function ParentView({ db, profile, school, terms }) {
  ...
  useEffect(() => {
    ...
    // Load only the parent's linked students
  ...
  useEffect(() => {
    ...
    // Load only published report cards for linked students
    const q = query(
      collection(db, 'reportCards'),
      where('studentId', 'in', linkedStudents.slice(0, 10)), // Firestore 'in' limit
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl">
      <h1 className="text-2xl font-semibold mb-6">Your child{students.length === 1 ? "'s" : "ren's"} report cards</h1>
      {students.length === 0 ? (
                ...
                {reports.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {reports.map(r => (
                      <button
                        key={r.id}
                        onClick={() => setViewing({ report: r, student: s })}
