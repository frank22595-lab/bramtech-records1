...
export default function ReportCardView({
  ...
  // Publish permission:
  //   - Directors & admins: can always publish
  //   - Teachers: can publish if the class is in their assigned classes
  //     (Publishing is a BASELINE permission for teachers — always on)
  const canPublish = isAdminOrDirector || canAccessClass(classId);
  ...
  const frozen =
    isPublished && existingSnapshot?.publishSnapshot
      ? {
  ...
  const attendancePct = timesOpened
    ? attendancePercentage(daysPresent, timesOpened)
    : null;
  const classTeacherName =
    existingSnapshot?.classTeacherName || classTeacher?.fullName || "";
        ...
        <Card className="max-w-md w-full" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="p-6 text-sm text-ink-soft space-y-2">
            <p>
              ...
              has no scores entered yet for this term.
            </p>
            <p>
              Enter scores under <strong>Result entry</strong> first, then the
              report card will be available here.
            </p>
          </div>
  ...
  const publish = async () => {
    ...
    try {
      ...
      setMsg("Publish failed: " + err.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  const unpublish = async () => {
    if (!confirm("Unpublish this report card? Parents will no longer see it."))
      return;
    setMsg("");
    setSaving(true);
    try {
  ...
  const buildPDF = async () => {
    ...
    return await generateReportCardPDF({
      ...
      psychomotor:
        config.showPsychomotor !== false
          ? existingSnapshot?.psychomotor || []
          : null,
      affective:
        config.showAffective !== false
          ? existingSnapshot?.affective || []
          : null,
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const filename =
        `${student.fullName.replace(/\s+/g, "_")}_${term?.academicYear || ""}_${term?.name || ""}.pdf`.replace(
          /\s+/g,
          "_",
        );
      pdf.save(filename);
  ...
  return (
    <>
      ...
      >
        <Card
          className="max-w-5xl w-full my-4"
        ...
        >
          <div className="p-4 md:p-5 border-b border-slate-200 flex items-center justify-between gap-3 sticky top-0 bg-white z-10">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-ink-soft">
                {className} · {term?.academicYear} {term?.name}
              </p>
            </div>
          ...
          <div className="p-4 md:p-6 space-y-5">
            <div className="text-center border-b border-slate-200 pb-4">
              ...
              {school?.address && (
                <div className="text-xs text-ink-soft">{school.address}</div>
              )}
              {school?.motto && (
            ...
            <div className="grid md:grid-cols-3 gap-3">
              <InfoPanel title="Student">
                ...
                {config.showRegNumber !== false && (
                  <Row label="Adm #" value={student.admissionNumber || "—"} />
                )}
                ...
                {config.showAge !== false && age !== null && (
                  <Row label="Age" value={`${age} years`} />
                )}
                {config.showWeightHeight && student.weight && (
                  <Row label="Weight" value={`${student.weight} kg`} />
                )}
                {config.showWeightHeight && student.height && (
                  <Row label="Height" value={`${student.height} cm`} />
                )}
              </InfoPanel>
              <InfoPanel title="Class">
                ...
                {config.showPosition !== false && source.overallPosition && (
                  <Row
                    label="Position"
                    value={`${ordinal(source.overallPosition)} of ${source.classSize}`}
                  />
                )}
                {config.showNoSubjects !== false && (
                  <Row label="Subjects" value={source.subjects.length} />
                )}
                {classTeacherName && (
                  <Row label="Class teacher" value={classTeacherName} />
                )}
                {term?.closingDate && (
                  <Row label="Term ended" value={term.closingDate} />
                )}
              </InfoPanel>
              {existingSnapshot?.attendance &&
                config.showAttendance !== false && (
                  <InfoPanel title="Attendance">
                    {config.showTimesOpened !== false && timesOpened && (
                      <Row
                        label="School opened"
                        value={`${timesOpened} days`}
                      />
                    )}
                    <Row label="Days present" value={daysPresent} />
                    <Row label="Days absent" value={daysAbsent} />
                    {config.showAttendancePercent !== false &&
                      attendancePct !== null && (
                ...
                <table className="w-full text-xs">
                  <thead className="bg-slate-100">
                    <tr>
                      ...
                      {config.showSubjectPosition !== false && (
                        <th className="px-2 py-1.5 text-center font-semibold">
                          Pos
                        </th>
                      )}
                      {config.showClassAverage !== false && (
                        <th className="px-2 py-1.5 text-center font-semibold hidden md:table-cell">
                          Avg
                        </th>
                      )}
                      {config.showClassHighest !== false && (
                        <th className="px-2 py-1.5 text-center font-semibold hidden md:table-cell">
                          High
                        </th>
                      )}
                      {config.showClassLowest !== false && (
                        <th className="px-2 py-1.5 text-center font-semibold hidden md:table-cell">
                          Low
                        </th>
                      )}
                  ...
                  <tbody>
                    {source.subjects.map((s) => {
                      ...
                      const arr = Array.isArray(s.assessments)
                        ? s.assessments
                        : Object.values(s.assessments);
                      return (
                        <tr
                          key={s.subjectId}
                        ...
                        >
                          ...
                          {config.showSubjectPosition !== false && (
                            <td className="px-2 py-1.5 text-center">
                              {s.classPosition ? ordinal(s.classPosition) : "-"}
                            </td>
                          )}
                          {config.showClassAverage !== false && (
                            <td className="px-2 py-1.5 text-center text-ink-soft hidden md:table-cell">
                              {s.classAverage}
                            </td>
                          )}
                          {config.showClassHighest !== false && (
                            <td className="px-2 py-1.5 text-center text-emerald-700 hidden md:table-cell">
                              {s.classHighest}
                            </td>
                          )}
                          {config.showClassLowest !== false && (
                            <td className="px-2 py-1.5 text-center text-red-600 hidden md:table-cell">
                              {s.classLowest}
                            </td>
                          )}
                          <td className="px-2 py-1.5 text-ink-soft hidden md:table-cell">
                            {s.remark}
                          </td>
                        </tr>
                      );
            ...
            <div className="grid grid-cols-2 md:grid-cols-6 gap-3 bg-slate-50 rounded-lg p-4">
              ...
              {config.showPosition !== false && source.overallPosition && (
                <Stat
                  label="Position"
                  value={ordinal(source.overallPosition)}
                />
              )}
            ...
            {existingSnapshot?.adviserComment && config.showAdviserComment && (
              <div>
                ...
                <div className="bg-slate-50 p-3 rounded text-sm">
                  {existingSnapshot.adviserComment}
                </div>
                <div className="text-xs text-ink-soft mt-1">
                  — {existingSnapshot.adviserName || "Academic Adviser"}
                </div>
              </div>
            )}
            {existingSnapshot?.classTeacherComment && (
              <div>
                ...
                <div className="text-xs text-ink-soft mt-1">
                  — {classTeacherName || existingSnapshot.classTeacherName}
                </div>
              </div>
            )}
            {existingSnapshot?.headTeacherComment && (
              <div>
                ...
                <div className="text-xs text-ink-soft mt-1 flex items-center gap-3 flex-wrap">
                  <span>
                    — {principalName || existingSnapshot.headTeacherName} (
                    {existingSnapshot.headTeacherTitle || principalTitle})
                  </span>
              ...
              !readOnly && (
                <Card className="p-4 bg-amber-50 border-amber-200 text-sm">
                  <p className="text-amber-900">
                    No traits or comments yet. Fill them in from{" "}
                    <strong>Terms → {term?.name} → Traits & comments</strong>.
                  </p>
                </Card>
              )}
          </div>
          ...
          {canPublish && !readOnly && (
            <div className="p-4 md:p-6 border-t border-slate-200 flex items-center justify-between gap-3 flex-wrap">
              {msg && (
                <span
                  className={`text-sm ${msg.includes("failed") || msg.includes("Failed") ? "text-red-600" : "text-emerald-700"}`}
                >
                  {msg}
                </span>
              )}
...
function getAssessmentCodes(source) {
  ...
  if (Array.isArray(first.assessments))
    return first.assessments.map((a) => a.code);
