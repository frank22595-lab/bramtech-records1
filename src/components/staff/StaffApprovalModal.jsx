...
import { doc, updateDoc, serverTimestamp, collection, getDocs, orderBy, query } from 'firebase/firestore'
...
export default function StaffApprovalModal({ staff, onClose, onSave }) {
  ...
  const [classTeacherOf, setClassTeacherOf] = useState(staff.classTeacherOf || staff.proposedClassTeacherOf || '')
  const [assignedClasses, setAssignedClasses] = useState(
    Array.isArray(staff.assignedClasses) && staff.assignedClasses.length > 0
      ? staff.assignedClasses
      : (staff.proposedClassTeacherOf ? [staff.proposedClassTeacherOf] : [])
  )
  const [assignedSubjects, setAssignedSubjects] = useState(
    Array.isArray(staff.assignedSubjects) && staff.assignedSubjects.length > 0
      ? staff.assignedSubjects
      : (Array.isArray(staff.proposedSubjects) ? staff.proposedSubjects : [])
  )
  ...
  const handleClassTeacherOfChange = (newClassId) => {
    ...
    // Also ensure that class is in assignedClasses
  ...
  const save = async (opts = { approve: false }) => {
    ...
    try {
      ...
      // First-time approval flips status pending → active
      ...
      >
        ...
        {loading ? (
          ...
          <div className="p-5 space-y-6">
            ...
            <div>
              ...
              {role === 'admin' && (
                <p className="text-xs text-amber-700 mt-2 bg-amber-50 p-2 rounded">
                  ⚠ Admins have almost the same access as directors. Only give this to trusted staff.
                </p>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-900 mb-2">
                Also teaches in classes <span className="text-slate-500 font-normal">(select all)</span>
              </label>
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-900 mb-2">
                Subjects taught <span className="text-slate-500 font-normal">(select all)</span>
              </label>
