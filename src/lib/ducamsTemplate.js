/**
 * Report card renderer — Classic (Ducams) design.
 *
 * v11 — hardened against incomplete data.
 * Every field access is guarded. If subjects are missing, the report
 * still renders showing overall grade + percentage + basic student info.
 */
...
function safeOrdinal(n) {
  if (n == null || n === "" || isNaN(Number(n))) return "—";
  try {
    return ordinal(Number(n));
  } catch {
    return String(n);
  }
}
...
function normalizeSubject(s) {
  const raw = s || {};
  let assessments = raw.assessments;
  if (!assessments) assessments = [];
  else if (!Array.isArray(assessments)) {
    try {
  ...
  return {
    subjectName: String(raw.subjectName || raw.name || ""),
    total: raw.total ?? 0,
    grade: String(raw.grade || ""),
    remark: String(raw.remark || ""),
    classPosition: raw.classPosition,
    classAverage: raw.classAverage,
    classHighest: raw.classHighest,
    classLowest: raw.classLowest,
    assessments: assessments.filter((a) => a && typeof a === "object"),
  };
}

function sortCodes(codes) {
  const isExam = (c) => /^EX|EXAM/i.test(String(c || ""));
  const clean = codes.filter((c) => c != null);
  return [...clean.filter((c) => !isExam(c)), ...clean.filter(isExam)];
}

function getAssessmentInfo(subjects) {
  if (!Array.isArray(subjects) || !subjects.length) return [];
  // Merge assessment definitions across all subjects (some may have partial data)
  const seen = new Map();
  subjects.forEach((s) => {
  ...
  return sortCodes([...seen.keys()]).map((code) => seen.get(code));
}

function scoreCell(subject, code) {
  const arr = subject.assessments || [];
  const a = arr.find((x) => x && x.code === code);
...
function autoNameFontSize(nameStr) {
  ...
  if (!s) return 17;
  if (s.length > 32) return 12;
  if (s.length > 25) return 14;
  if (s.length > 18) return 16;
  return 17;
}
...
export function renderDucams(input) {
  ...
  const report = {
    ...rawReport,
    subjects: (Array.isArray(rawReport.subjects) ? rawReport.subjects : []).map(
      normalizeSubject,
    ),
    totalObtained: rawReport.totalObtained ?? rawReport.totalScore ?? 0,
    percentageAverage: rawReport.percentageAverage ?? 0,
    overallPosition: rawReport.overallPosition,
    overallGrade: rawReport.overallGrade || "",
    overallRemark: rawReport.overallRemark || "",
    classSize: rawReport.classSize ?? 0,
    classAverage: rawReport.classAverage ?? 0,
    classHighestPct: rawReport.classHighestPct,
    classLowestPct: rawReport.classLowestPct,
  };
  const attendance = input.attendance || {};
  const psychomotor = Array.isArray(input.psychomotor) ? input.psychomotor : [];
  const affective = Array.isArray(input.affective) ? input.affective : [];
  const adviser = input.adviser || {};
  const classTeacher = input.classTeacher || {};
  const headTeacher = input.headTeacher || {};
  ...
  function drawLogoPlaceholder() {
    ...
    if (school?.shortName) {
      ...
      doc.text(
        String(school.shortName).toUpperCase().slice(0, 8),
        logoCX,
        logoCY + 1.5,
        { align: "center" },
      );
  ...
  if (hasPhoto) {
    try {
      doc.addImage(
        images.studentPhoto,
        detectImageFormat(images.studentPhoto),
        photoX,
        photoY,
        photoW,
        photoH,
  ...
  const nameLines = nameStr ? doc.splitTextToSize(nameStr, infoWidth) : [];
  ...
  if (school?.address) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9.5);
    const addrLines = doc.splitTextToSize(String(school.address), infoWidth);
  ...
  if (school?.motto) {
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(brand.r, brand.g, brand.b);
    const mottoLines = doc.splitTextToSize(`"${school.motto}"`, infoWidth);
    if (mottoLines[0])
      doc.text(mottoLines[0], infoCenter, ny, { align: "center" });
    doc.setTextColor(0);
  }
  ...
  const drawInfoPanel = (x, py, w, h, rows) => {
    ...
    rows.forEach(([label, value], i) => {
      ...
      const truncated =
        (doc.splitTextToSize(val, w * 0.5 - 3) || [""])[0] || "";
  ...
  drawInfoPanel(M, y, p1w, row1H, [
    ...
    ["Class", className || student.currentClass || ""],
  ...
  drawInfoPanel(M + p1w + gap, y, p1w, row1H, [
    ...
    [
      "Term",
      term?.termNumber ? safeOrdinal(term.termNumber) : term?.name || "",
    ],
  ...
  drawInfoPanel(M, y, p1w, row2H, [
    ["Overall total score", String(report.totalObtained ?? 0)],
    ["Student's average", `${report.percentageAverage ?? 0}%`],
    [
      "Position in class",
      report.overallPosition ? safeOrdinal(report.overallPosition) : "—",
    ],
    ["Overall grade", report.overallGrade || "—"],
    ["Overall performance", report.overallRemark || ""],
  ]);
  drawInfoPanel(M + p1w + gap, y, p1w, row2H, [
    ["Class size", String(report.classSize ?? 0)],
    ["Class average score", `${report.classAverage ?? 0}%`],
    [
      "Highest in class",
      report.classHighestPct != null
        ? `${report.classHighestPct}%`
        : `${report.percentageAverage ?? 0}%`,
    ],
    [
      "Lowest in class",
      report.classLowestPct != null ? `${report.classLowestPct}%` : "—",
    ],
  ...
  const attItems = [
    ["Days school opened", attendance?.timesOpened ?? attendance?.total ?? "—"],
    ["Days present", attendance?.daysPresent ?? attendance?.present ?? "—"],
    ["Days absent", attendance?.daysAbsent ?? attendance?.absent ?? "—"],
    [
      "Attendance rate",
  ...
  if (badgeH > 4) {
    ...
    doc.setFontSize(9);
    doc.text(
      promoted ? "PROMOTED" : "RESULT ISSUED",
      rightX + p1w / 2,
      badgeY + badgeH / 2 + 1.5,
      { align: "center" },
    );
  ...
  const assessInfo = getAssessmentInfo(report.subjects);
  ...
  const assessLabel = (a) => {
    const raw = String(a.name || a.code || "");
    const display = raw.length > 16 ? String(a.code || "") : raw;
  ...
  const realRows = report.subjects.map((s) => [
    ...
    s.grade || "-",
    ...(showCols.pos
      ? [s.classPosition ? safeOrdinal(s.classPosition) : "-"]
      : []),
  ...
  const commentCount =
    (showAdviser && adviser?.comment ? 1 : 0) +
  ...
  const perRow = Math.max(
    4,
    Math.min(7, (tableSpace - 14) / TARGET_SUBJECT_ROWS),
  );
  const fontSize =
    perRow >= 6 ? 7.5 : perRow >= 5 ? 7 : perRow >= 4.5 ? 6.5 : 6;
  ...
  const bottomY = y;
  const psy = psychomotor.filter((s) => s && s.rating > 0);
  const aff = affective.filter((s) => s && s.rating > 0);
  ...
  const drawSimpleTable = (x, sy, w, title, rows) => {
    ...
    rows.forEach(([label, val], i) => {
      ...
      doc.setFontSize(6.4);
      doc.setTextColor(0);
      const lines = doc.splitTextToSize(String(label), w - 8) || [""];
      doc.text(lines[0] || "", x + 2, ry + 2.2);
      doc.setFont("helvetica", "bold");
  ...
  const drawEmptyStateSection = (x, sy, w, title) => {
    ...
    doc.text("Not recorded", x + w / 2, ry + emptyH / 2 + 0.5, {
      align: "center",
    });
    doc.setTextColor(0);
    ry += emptyH;
  ...
  const grading = Array.isArray(school?.gradingScale)
    ? school.gradingScale
    : [];
  if (grading.length > 0) {
    doc.setFillColor(brand.r, brand.g, brand.b);
    ...
    grading.slice(0, 9).forEach((g) => {
      ...
      doc.text(`${g.min ?? 0}-${g.max ?? 100}%`, col3X + 1, c3Y + 1.9);
      doc.setFont("helvetica", "bold");
      ...
      doc.text(remarkLines[0] || "", col3X + gw1 + gw2 + gw3 + 1, c3Y + 1.9);
      doc.setTextColor(0);
      c3Y += gRowH;
    });
    doc.setDrawColor(GRID[0], GRID[1], GRID[2]);
    ...
    for (let r = 5; r >= 1; r--) {
      const idx = 5 - r;
      doc.setFillColor(
        idx % 2 === 0 ? ALT[0] : PANEL[0],
        idx % 2 === 0 ? ALT[1] : PANEL[1],
        idx % 2 === 0 ? ALT[2] : PANEL[2],
      );
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(5.4);
      const meaning = doc.splitTextToSize(
        RATING_MEANINGS[r],
        col3W - rw1 - 2,
      ) || [""];
      doc.text(meaning[0] || "", col3X + rw1 + 1, c3Y + 1.9);
      c3Y += rRowH;
    }
    doc.setDrawColor(GRID[0], GRID[1], GRID[2]);
    doc.setLineWidth(0.2);
  ...
  if (showAdviser && adviser?.comment) {
    commentRows.push({
      label: "Academic adviser's report",
      comment: adviser.comment,
      signer: adviser.name || "Academic Adviser",
    });
  }
  if (config.showTeacherComment !== false && classTeacher?.comment) {
    commentRows.push({
      label: "Class teacher's report",
      comment: classTeacher.comment,
      signer: classTeacher.name,
    });
  }
  if (config.showHeadTeacherComment !== false && headTeacher?.comment) {
    commentRows.push({
      label: `${headTeacher.title || "Principal"}'s report`,
      comment: headTeacher.comment,
      signer: headTeacher.name,
    });
  }
  ...
  if (commentsSpace >= 5 && commentRows.length > 0) {
    ...
    commentRows.forEach((row, i) => {
      ...
      if (row.signer) {
        ...
        doc.text(`— ${row.signer}`, M + W - 2, cy + rowH - 1.5, {
          align: "right",
        });
        doc.setTextColor(0);
      }
  ...
  if (remainingFooter >= 8 && (hasSignature || hasStamp)) {
    if (hasSignature) {
      ...
      try {
        doc.addImage(
          images.signature,
          detectImageFormat(images.signature),
          sigX,
          sigY,
          sigW,
          sigH,
        ...
        doc.setFontSize(6.5);
        doc.setTextColor(80);
        doc.text("Principal's signature", sigX + sigW / 2, sigY + sigH + 3, {
          align: "center",
        });
        doc.setTextColor(0);
      } catch (err) {
    ...
    if (hasStamp) {
      ...
      try {
        doc.addImage(
          images.stamp,
          detectImageFormat(images.stamp),
          stampX,
          stampY,
          stampSize,
          stampSize,
