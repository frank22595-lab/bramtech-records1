...
function SchoolApp() {
  ...
  return (
    <Routes>
      ...
      <Route
        path="/staff-join"
        element={user ? <Navigate to="/dashboard" replace /> : <StaffJoinPage />}
      />
      ...
      <Route path="/dashboard" element={<AdminRoute user={user} profile={profile} school={school}><Dashboard /></AdminRoute>} />
      <Route path="/school" element={<AdminRoute user={user} profile={profile} school={school}><SchoolPage /></AdminRoute>} />
      <Route path="/school/:tab" element={<AdminRoute user={user} profile={profile} school={school}><SchoolPage /></AdminRoute>} />
      <Route path="/terms" element={<AdminRoute user={user} profile={profile} school={school}><TermsListPage /></AdminRoute>} />
      <Route path="/terms/:termId" element={<AdminRoute user={user} profile={profile} school={school}><TermWorkspace /></AdminRoute>} />
      <Route path="/terms/:termId/:tab" element={<AdminRoute user={user} profile={profile} school={school}><TermWorkspace /></AdminRoute>} />
      <Route path="/reports" element={<AdminRoute user={user} profile={profile} school={school}><ReportsBrowsePage /></AdminRoute>} />
      <Route path="/reports/design" element={<AdminRoute user={user} profile={profile} school={school}><ReportDesignPage /></AdminRoute>} />
      <Route path="/settings" element={<AdminRoute user={user} profile={profile} school={school}><SettingsPage /></AdminRoute>} />
      <Route path="/settings/:tab" element={<AdminRoute user={user} profile={profile} school={school}><SettingsPage /></AdminRoute>} />
      <Route path="/setup/*" element={<SetupRoute user={user} profile={profile} />} />
...
function AdminRoute({ user, profile, school, children }) {
  ...
  if (profile?.status === 'pending') return <Navigate to="/staff-pending" replace />
...
function SetupInProgress() {
  return (
    <div className="max-w-lg mx-auto mt-24 text-center px-4">
      ...
      <p className="text-ink-soft">Your school director is still setting up the portal. Please check back soon.</p>
    </div>
  )
}
